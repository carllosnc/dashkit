...
export function Sidebar({
  ...
  const [completedPath, setCompletedPath] = React.useState(currentPath);
    ...
    <>
      {sections.map((section, idx) => (
        <div key={idx} className="mb-10 last:mb-0">
          <h4 className="text-[11px] font-bold text-muted-foreground uppercase tracking-[0.15em] mb-4">
            {section.title}
          </h4>
          <nav className="flex flex-col gap-1">
            {section.links.map((link) => {
              const active = checkIsActive(link.to);
              const isColorActive = active && completedPath === link.to;
