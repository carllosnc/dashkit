    ...
    <div className={cn("flex flex-wrap gap-2 sm:gap-3 items-center justify-start", containerClassName)}>
      {Array.from({ length }).map((_, i) => (
        <input
          ...
          disabled={disabled}
          className={cn(
            "w-10 h-12 sm:w-11 sm:h-13 text-center text-lg sm:text-xl font-bold bg-background text-foreground border border-input rounded-[var(--radius)] outline-none transition-all duration-300 focus:ring-2 focus:ring-ring focus:ring-offset-2 placeholder:text-muted-foreground disabled:cursor-not-allowed disabled:opacity-50",
